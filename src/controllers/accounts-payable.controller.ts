...
class AccountsPayableController {
	...
	public async create(
		...
		try {
			const accountPayable = await createAccountPayableService({
				...data,
				userId,
			});
		...
		try {
			...
			const accounts = await listAccountsPayableService({
				...
				documentDateStart: documentDateStart
					? new Date(documentDateStart)
					: undefined,
				documentDateEnd: documentDateEnd
					? new Date(documentDateEnd)
					: undefined,
			});
	...
	public async delete(
		...
		try {
			...
			return reply
				.status(204)
				.send({ message: "Conta a pagar removida com sucesso" });
		} catch (error) {
			if (error instanceof AppError) {
				return reply.status(400).send({ message: error.message });
			} else {
	...
	public async reversePayment(request: FastifyRequest, reply: FastifyReply) {
		try {
			const { id } = request.params as { id: string };
			const { reason } = request.body as { reason: string };

			const reverseAccountPayableUseCase = makeReverseAccountPayableUseCase();
			const result = await reverseAccountPayableUseCase(id, reason);
