...
import { deleteInvoiceService } from "@/services/invoice/delete-invoice.service";
import {
...
class InvoiceController {
	async create(
		request: FastifyRequest<CreateInvoiceRequest>,
		reply: FastifyReply,
	) {
		try {
			const {
	...
	async update(
		request: FastifyRequest<UpdateInvoiceRequest>,
		reply: FastifyReply,
	) {
		try {
	...
	async list(
		request: FastifyRequest<ListInvoicesRequest>,
		reply: FastifyReply,
	) {
		try {
			const { customerId, bankAccountId, issueDateStart, issueDateEnd } =
				request.query;
	...
	async delete(
		request: FastifyRequest<DeleteInvoiceRequest>,
		reply: FastifyReply,
	) {
		try {
			const { id } = request.params;
