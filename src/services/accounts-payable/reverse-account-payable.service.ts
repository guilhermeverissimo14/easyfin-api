...
export const reverseAccountPayableService = async (id: string, reason: string) => {
   ...
   if (account.status !== PaymentStatus.PAID) {
      throw new AppError('Apenas contas com status PAID podem ser estornadas', 400)
   }
   ...
   try {
      ...
      await prisma.$transaction(async (prisma) => {
         ...
         await prisma.accountsPayable.update({
            ...
            data: {
               ...
               observation: `${account.observation || ''} [ESTORNADO: ${reason}]`.trim(),
            },
         })
      })
      ...
      throw new AppError('Erro interno do servidor ao estornar conta a pagar', 500)
   }
}
