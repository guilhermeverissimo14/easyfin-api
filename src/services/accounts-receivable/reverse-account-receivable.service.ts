...
import { recalculateCashFlowBalances, recalculateCashBoxFlowBalances } from '@/utils/recalculate-cash-flow-balances'
import { prisma } from '@/lib/prisma'

export const reverseAccountReceivableService = async (id: string, reason?: string) => {
   ...
   if (account.status !== PaymentStatus.PAID) {
      throw new AppError('Esta conta não está liquidada e não pode ser estornada', 400)
   }

   if (!account.receiptDate) {
      throw new AppError('Conta sem data de recebimento não pode ser estornada', 400)
   }
   ...
   try {
      const reversedAccount = await prisma.$transaction(
         async (prisma) => {
            ...
            if (account.bankAccountId) {
               ...
               const bankTransaction = await prisma.bankTransactions.findFirst({
                  where: {
                     ...
                     transactionAt: {
                        gte: new Date(account.receiptDate!.getTime() - 24 * 60 * 60 * 1000),
                        lte: new Date(account.receiptDate!.getTime() + 24 * 60 * 60 * 1000),
                     },
                  },
               ...
               if (cash) {
                  ...
                  const cashTransaction = await prisma.cashTransaction.findFirst({
                     where: {
                        ...
                        amount: receivedAmount,
                        transactionAt: {
                           gte: new Date(account.receiptDate!.getTime() - 72 * 60 * 60 * 1000),
                           lte: new Date(account.receiptDate!.getTime() + 72 * 60 * 60 * 1000),
                        },
                     },
                     orderBy: { createdAt: 'desc' },
                  })
